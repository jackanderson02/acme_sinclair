...
func (repo *PostgresUserRepository) GetProducts() ([]model.Product, error){

	DB := repo.DB
	products := []model.Product{}
	err := sqlx.Select(DB, &products, "SELECT * FROM products")

	if err != nil {
		fmt.Println("Could not load products.", err)
		return []model.Product{}, errors.New("Failed to load products from database.")
	}
